...
static mut BUFFER_HANDLE: *mut u8 = std::ptr::null_mut();
...
impl<'a> Sim {
    fn to_ptr(self) -> *mut Sim {
        let sim_boxed = Box::new(self);
        Box::into_raw(sim_boxed)
    }

    fn from_ptr(ptr: *mut Sim) -> &'a mut Sim {
        unsafe { &mut *ptr }
    }
    ...
    fn init_sim() -> Result<Sim, String> {
        ...
        let dims = dim4!(128, 128, 4);
        let output = constant::<u8>(0u8, dims);
        let elements = output.elements();
        let mut results = vec!(0u8; output.elements());
        unsafe { BUFFER_HANDLE = results.as_mut_ptr(); }

        Ok(Sim { results: results, output: output, elements: elements })
    }

    fn simulate(&mut self) -> *mut u8 {
        // a.host(&mut (*self).results);
        let dims = dim4!(128, 128, 4);
        let mut output = constant::<u8>(0u8, dims);
        let elements = output.elements();
        let mut results = vec!(0u8; elements);

        // lbm_d2q9(&mut output);

        output.cast::<u8>().host(&mut results);
        results.as_mut_ptr()
    }

    fn get_results(&mut self) -> *mut u8 {
        unsafe { BUFFER_HANDLE }
    }
}

#[no_mangle]
pub extern "C" fn init_sim(ptr: *mut *const Sim) -> bool {
    match Sim::init_sim() {
        Ok(sim) => {
            unsafe {
                *ptr = sim.to_ptr();
...
#[no_mangle]
pub extern "C" fn init_output(ptr: *mut Sim, data_ptr: *mut *mut u8) {
    if !ptr.is_null() {
        unsafe {
            *data_ptr = BUFFER_HANDLE;
        }
    }
}

#[no_mangle]
pub extern "C" fn start_simulation(ptr: *mut Sim, data_ptr: *mut *mut u8) {
    if !ptr.is_null() {
        unsafe {
            *data_ptr = Sim::from_ptr(ptr).simulate();
        }
    }
}

#[no_mangle]
pub extern "C" fn get_sim_data(ptr: *mut Sim, data_ptr: *mut *mut u8) {
    if !ptr.is_null() {
        unsafe {
            *data_ptr = Sim::from_ptr(ptr).get_results();
        }
    }
}
