...
impl<'a> Sim {
    ...
    fn get_results_ptr(&self) -> *mut u8 {
        self.results_ptr
    }

    fn init_sim_lid() -> Result<Sim, String> {
        set_device(0);
    ...
    fn init_sim_channel() -> Result<Sim, String> {
        set_device(0);
        set_backend(Backend::OPENCL);
        // Grid length, number and spacing
        let nx: u64 = 300;
...
pub extern "C" fn init_sim(ptr: *mut *mut Sim, data_ptr: *mut *mut u8) -> bool {
    match Sim::init_sim_channel() {
        Ok(sim) => {
            unsafe {
                *ptr = sim.to_ptr();
                *data_ptr = Sim::from_ptr(*ptr).get_results_ptr();
            }
...
// #[no_mangle]
// pub extern "C" fn start_simulation_single_threaded(ptr: *mut Sim) {
//     if !ptr.is_null() {
//         Sim::from_ptr(ptr).simulate_single_thread();
//     }
// }
